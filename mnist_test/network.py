...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None):
        """Train the neural network using mini-batch SGD. The ``training_data`` is a
        list of tuples ``(x, y)`` representing the training inputs and the desired
        outputs. The other non-optional parameters are self-explanatory. If
        ``test_data`` is provided then the network will be evaluated against the
        ...
        print("Zipped training_data: {}".format(training_data))
        training_data = [x for x in training_data]
        print("List training_data: {}".format(training_data))
    ...
    def update_mini_batch(self, mini_batch, eta):
        ...
        gradient descent using backpropogation to a single
    ...
    def evaluate(self, test_data):
        ...
        outputs the correct results. Note that the nn's output is
        assumed to be the index of whichever neuron in the final
        layer has the highest activations.
        """
        test_results = [(np.argmax(self.feedforward(x)), y) for
                (x, y) in test_data]
